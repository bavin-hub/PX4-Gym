...
from px4_msgs.msg import OffboardControlMode, VehicleCommand, \
                            ...
                            VehicleGlobalPosition, VehicleAttitudeSetpoint, HoverThrustEstimate
import numpy as np
...
class OffboardControl(Node):
    ...
    def __init__(self) -> None:
        ...
        self.vehicle_hover_thrust_subscriber = self.create_subscription(
            HoverThrustEstimate, '/fmu/out/hover_thrust_estimate', self.vehicle_hover_callback, qos_profile)
        ...
        # Latest attitude in the policy frame (roll, pitch, yaw in FLU/NWU).
        # The delta policy adds its roll/pitch/yaw outputs to this value.
        self.current_euler_flu = torch.zeros(3, dtype=torch.float32, device=self.device)
    ...
    def create_observation(self, curr_wp):
        ...
        # Training observes the previous processed controller command:
        # [controller_thr, roll_cmd, pitch_cmd, yaw_cmd], not the raw delta output.
        ...
        current_roll = torch.atan2(
            2.0 * (qw * qx + qy * qz),
            1.0 - 2.0 * (qx * qx + qy * qy),
        )
        sin_pitch = torch.clamp(
            2.0 * (qw * qy - qz * qx),
            -1.0,
            1.0,
        )
        current_pitch = torch.asin(sin_pitch)
        current_yaw = torch.atan2(
            2.0 * (qw * qz + qx * qy),
            1.0 - 2.0 * (qy * qy + qz * qz),
        )
        # Save the current absolute attitude before policy eval so scale_and_clip()
        # can convert delta roll/pitch/yaw into absolute setpoints.
        self.current_euler_flu = torch.stack([current_roll, current_pitch, current_yaw])
        ...
        observation = torch.cat([
            ...
            previous_action,          # [13:17] previous controller thrust, roll, pitch, yaw
    ...
    def scale_and_clip(self, action):
        # Match delta-attitude training. The policy outputs raw deltas in [-1, 1],
        # which are converted to processed controller commands:
        # [controller_thr, roll_cmd, pitch_cmd, yaw_cmd] in FLU/NWU.
        clipped_action = torch.clamp(action, -1.0, 1.0).clone()

        px4_hover_thrust = -0.6
        px4_hover_thrust = -self.vehicle_hover_thrust.hover_thrust
        print('from px4 : ', px4_hover_thrust)
        px4_min_thrust = -0.9
        px4_max_thrust = -0.4
        delta_thrust_scale = 0.1
        max_delta_angle = torch.deg2rad(torch.tensor(5.0, device=action.device))

        # Policy thrust is a delta around PX4 hover thrust. PX4 attitude thrust
        # uses negative z thrust, so hover is -0.6 and stronger thrust is more negative.
        delta_thrust = clipped_action[:, 0] * delta_thrust_scale
        print("integrated thrust : ", px4_hover_thrust + delta_thrust)
        px4_thrust = torch.clamp(
            px4_hover_thrust + delta_thrust,
            px4_min_thrust,
            px4_max_thrust,
        )
        print()
        # Store the same processed thrust convention used by training observations.
        controller_thr = torch.clamp(
            (-px4_thrust / abs(px4_hover_thrust)) - 1.0,
            -1.0,
            1.0,
        )

        # Convert delta attitude outputs into absolute attitude commands.
        current_euler = self.current_euler_flu.to(device=action.device, dtype=action.dtype)
        roll_cmd = current_euler[0] + clipped_action[:, 1] * max_delta_angle
        pitch_cmd = current_euler[1] + clipped_action[:, 2] * max_delta_angle
        yaw_cmd = torch.atan2(
            torch.sin(current_euler[2] + clipped_action[:, 3] * max_delta_angle),
            torch.cos(current_euler[2] + clipped_action[:, 3] * max_delta_angle),
        )

        attitude_command = torch.stack(
            [controller_thr, roll_cmd, pitch_cmd, yaw_cmd],
            dim=1,
        )

        # Observation indices [13:17] use the previous processed controller command.
        self.prev_action = attitude_command[0].detach().cpu().tolist()

        # Return PX4-facing commands while keeping prev_action in training format above.
        thrust_z = px4_thrust
        roll_body = roll_cmd
        pitch_body = -pitch_cmd
        yaw_body = -yaw_cmd

        return torch.stack(
            [thrust_z, roll_body, pitch_body, yaw_body],
            dim=1,
        )
    ...
    def policy_eval(self, curr_wp):
        ...
        # EXPERIMENTAL: rotate the roll/pitch command pair from body FRD axes
        # into world NED axes. This is intentionally isolated here so it can be
        # reverted by returning action[1], action[2] directly again.
        thrust_z, roll_body, pitch_body, yaw_body = action
        # q = self.vehicle_odometry.q
        # qw, qx, qy, qz = q
        # yaw = torch.atan2(
        #     torch.tensor(2.0 * (qw * qz + qx * qy), dtype=torch.float32),
        #     torch.tensor(1.0 - 2.0 * (qy * qy + qz * qz), dtype=torch.float32),
        # ).item()
        # cos_yaw = torch.cos(torch.tensor(yaw)).item()
        # sin_yaw = torch.sin(torch.tensor(yaw)).item()
        # roll_world_ned = cos_yaw * roll_body - sin_yaw * pitch_body
        # pitch_world_ned = sin_yaw * roll_body + cos_yaw * pitch_body

        return float(thrust_z), float(roll_body), float(pitch_body), float(yaw_body)
        # return float(thrust_z), float(roll_world_ned), float(pitch_world_ned), float(yaw_body)
    ...
    def publish_attitude_setpoint(self, t: float, r: float, p: float, y: float):
    ...
    def timer_callback(self) -> None:
        ...
        if self.home_lock is True:
            if self.vehicle_status.arming_state != VehicleStatus.ARMING_STATE_ARMED:
                ...
                print("i am here")
                if self.vehicle_hover_thrust.hover_thrust != None and self.vehicle_hover_thrust.hover_thrust > 0.0:
