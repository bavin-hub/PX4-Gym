"""Train the Aerial Gym-port X500 velocity task with RL-Games."""
...
parser.add_argument("--config", type=Path, default=ROOT / "config/rl_games_x500_velocity.yaml")
...
from aerial_isaac_lab.tasks import TASK_ID, X500VelocityEnvCfg

with args.config.open() as stream:
    agent_cfg = yaml.safe_load(stream)
env_cfg = X500VelocityEnvCfg()
