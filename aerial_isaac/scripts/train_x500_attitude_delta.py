"""Train the Aerial Gym-port X500 attitude-delta task with RL-Games."""
...
parser.add_argument("--config", type=Path, default=ROOT / "config/rl_games_x500_attitude_delta.yaml")
...
from aerial_isaac_lab.tasks import X500_ATTITUDE_DELTA_TASK_ID, X500AttitudeDeltaEnvCfg

with args.config.open() as stream:
    agent_cfg = yaml.safe_load(stream)
env_cfg = X500AttitudeDeltaEnvCfg()
env_cfg.scene.num_envs = args.num_envs
env_cfg.sim.device = args.device
env = gym.make(X500_ATTITUDE_DELTA_TASK_ID, cfg=env_cfg)
